        ...
        <div style={{background:"#0a0a14",border:"1px solid #222",borderRadius:16,padding:28}}>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>onStart({n, secs:mins*60, names:names.slice(0,n)})}
              style={{flex:1,padding:14,borderRadius:12,background:"#FF6B6B",border:"none",color:"#fff",fontFamily:"'Courier New',monospace",fontSize:15,fontWeight:900,letterSpacing:2,cursor:"pointer"}}>
              START →
            </button>
          </div>
        </div>
...
function RoundSummary({ roundResult, onContinue }) {
  return (
          ...
          {roundResult.map((r, i) => (
            <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                ...
                <span style={{color:r.color,fontWeight:900,fontSize:16}}>+{r.earned} pts</span>
              </div>
            </div>
          ))}
        </div>
